...
| [`version-coupling.md`](version-coupling.md) | Runtime `__version__` is single-sourced from package metadata (`importlib.metadata.version`), no longer a hand-maintained literal; `pyproject.toml` `[project].version` (canonical) and the `README.md` citation block remain manual. `tests/test_version_consistency.py` enforces runtime == packaged. |
