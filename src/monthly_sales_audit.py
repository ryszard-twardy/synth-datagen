...
from .monthly_sales_profile import MonthlySalesBadDataProfile, MonthlySalesNormalizedAuditProfile
...
def _inject_normalized_nulls(
    ...
    targets = [("dim_customers", "city"), ("dim_customers", "segment"), ("dim_promotions", "promo_name")]
...
def _inject_normalized_negatives(
    ...
    for (table_name, column_name), count in zip(targets, allocations, strict=False):
        ...
        if column_name == "qty":
            frame.loc[picks, column_name] = -values.abs().fillna(1).clip(lower=1).round().astype(int)
        else:
...
def _inject_normalized_outliers(
    ...
    for (table_name, column_name), count in zip(targets, allocations, strict=False):
        ...
        frame = tables[table_name]
        picks = rng.choice(len(frame), size=count, replace=False)
        values = pd.to_numeric(frame.loc[picks, column_name], errors="coerce").abs().fillna(25.0)
...
def apply_monthly_audit_bad_data(
    ...
    if not audit.enabled:
        return exported, {}
    summary = {
        "null_required_fields": _inject_normalized_nulls(exported, audit.normalized, rng),
        "negative_numeric_values": _inject_normalized_negatives(exported, audit.normalized, rng),
        "monetary_outliers": _inject_normalized_outliers(exported, audit.normalized, rng),
    }
...
def apply_flat_audit_bad_data(
    ...
    summary["duplicate_order_ids"] = {"actual_count": duplicate_count, "requested_count": duplicate_count}

    remaining_defects = {
        "bad_orderdate_formats": _count_from_rate(original_len, audit.flat.bad_orderdate_rate),
        "mixed_ordervalue_formats": _count_from_rate(original_len, audit.flat.mixed_ordervalue_format_rate),
        "null_required_fields": _count_from_rate(original_len, audit.flat.null_required_rate),
        "negative_ordervalue": _count_from_rate(original_len, audit.flat.negative_ordervalue_rate),
    }
    ...
    summary["bad_orderdate_formats"] = {"actual_count": bad_dates_count, "requested_count": bad_dates_count}
    ...
    summary["null_required_fields"] = {"actual_count": null_count, "requested_count": null_count}

    negative_count = remaining_defects["negative_ordervalue"]
    if negative_count:
        picks = row_order[cursor:cursor + negative_count]
        cursor += negative_count
        values = pd.to_numeric(dirty.loc[picks, "OrderValue"], errors="coerce").abs().fillna(1.0)
        dirty.loc[picks, "OrderValue"] = (-values).round(2)
    summary["negative_ordervalue"] = {"actual_count": negative_count, "requested_count": negative_count}
    ...
    if mixed_count:
        ...
        for idx in picks.tolist():
            value = abs(float(pd.to_numeric(pd.Series([dirty.at[idx, "OrderValue"]]), errors="coerce").fillna(0.0).iloc[0]))
            dirty.at[idx, "OrderValue"] = _format_order_value_variant(value if value > 0 else 1.0, rng)
    summary["mixed_ordervalue_formats"] = {"actual_count": mixed_count, "requested_count": mixed_count}
