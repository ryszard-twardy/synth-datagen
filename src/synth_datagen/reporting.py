...
def write_data_dictionary(
    graph: SchemaGraph, config: GeneratorConfig, console: Console | None = None
) -> None:
    lines = [
        "# Data Dictionary",
        "",
    ...
    for table in graph.topological_order():
        ...
        for column in table.columns:
            pk_tag = (
                ...
                else (" FK" if column.name in table.fk_columns else "")
            )
            lines.append(f"        {column.dtype.value} {column.name}{pk_tag}")
        lines.append("    }")
    for relation in graph.relations:
        lines.append(
            f'    {relation.target_table} ||--o{{ {relation.source_table} : "has"'
        )
    lines.extend(["```", ""])
