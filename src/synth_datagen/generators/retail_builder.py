...
_SEGMENT_ORDER_WEIGHT_MULTIPLIERS = {
    "B2C": 1.0,
    "B2B": 1.6,
    "Premium": 2.0,
    "Budget": 0.35,
...
def build_retail_schema(
    config: GeneratorConfig,
) -> tuple[list[TableConfig], list[RelationConfig]]:
    ...
    tables = [
        TableConfig(
            ...
            columns=[
                ColumnConfig(
                    name="customer_id", dtype=DType.VARCHAR, nullable=False, unique=True
                ),
                ColumnConfig(
                    name="first_name",
                    dtype=DType.VARCHAR,
                ...
                ColumnConfig(
                    name="last_name", dtype=DType.VARCHAR, nullable=False, max_length=80
                ),
                ColumnConfig(
                    name="email", dtype=DType.VARCHAR, nullable=False, unique=True
                ),
                ColumnConfig(
                    name="phone", dtype=DType.VARCHAR, nullable=True, max_length=40
                ),
                ColumnConfig(
                    name="city", dtype=DType.VARCHAR, nullable=False, max_length=80
                ),
        ...
        TableConfig(
            ...
            columns=[
                ColumnConfig(
                    name="product_id", dtype=DType.VARCHAR, nullable=False, unique=True
                ),
                ColumnConfig(
                    name="sku", dtype=DType.VARCHAR, nullable=False, unique=True
                ),
                ColumnConfig(
                    name="name", dtype=DType.VARCHAR, nullable=False, max_length=160
                ),
                ...
                ColumnConfig(
                    name="brand", dtype=DType.VARCHAR, nullable=True, max_length=120
                ),
        ...
        TableConfig(
            ...
            columns=[
                ColumnConfig(
                    name="store_id", dtype=DType.VARCHAR, nullable=False, unique=True
                ),
                ColumnConfig(
                    name="store_name",
                    dtype=DType.VARCHAR,
                ...
                ColumnConfig(name="country", dtype=DType.VARCHAR, nullable=False),
                ColumnConfig(
                    name="region", dtype=DType.VARCHAR, nullable=False, max_length=80
                ),
        ...
        TableConfig(
            ...
            columns=[
                ColumnConfig(
                    name="date_id", dtype=DType.INT, nullable=False, unique=True
                ),
                ...
                ColumnConfig(
                    name="day_name", dtype=DType.VARCHAR, nullable=False, max_length=20
                ),
        ...
        TableConfig(
            ...
            columns=[
                ColumnConfig(
                    name="promo_id", dtype=DType.VARCHAR, nullable=False, unique=True
                ),
                ColumnConfig(
                    name="promo_name",
                    dtype=DType.VARCHAR,
        ...
        TableConfig(
            ...
            columns=[
                ColumnConfig(
                    name="order_id", dtype=DType.VARCHAR, nullable=False, unique=True
                ),
        ...
        TableConfig(
            ...
            columns=[
                ColumnConfig(
                    name="item_id", dtype=DType.VARCHAR, nullable=False, unique=True
                ),
        ...
        TableConfig(
            ...
            columns=[
                ColumnConfig(
                    name="payment_id", dtype=DType.VARCHAR, nullable=False, unique=True
                ),
                ColumnConfig(
                    name="order_id", dtype=DType.VARCHAR, nullable=False, unique=True
                ),
        ...
        TableConfig(
            ...
            columns=[
                ColumnConfig(
                    name="bridge_id", dtype=DType.VARCHAR, nullable=False, unique=True
                ),
                ColumnConfig(name="order_id", dtype=DType.VARCHAR, nullable=False),
    ...
    relations = [
        ...
        RelationConfig(
            source_table="fact_orders",
            source_column="date_id",
            target_table="dim_date",
            target_column="date_id",
        ),
        RelationConfig(
        ...
        RelationConfig(
            source_table="bridge_order_promotions",
            source_column="order_id",
            target_table="fact_orders",
            target_column="order_id",
        ),
...
class RetailDataBuilder:
    def __init__(
        ...
        *,
        ...
        self.faker = faker
        self.discount_seed = (
            config.discount_seed if config.discount_seed is not None else config.seed
        )
        self.discount_rng = (
            build_discount_rng(self.discount_seed)
            if config.discount_variation
            else None
        )
        self.range_start, self.range_end = resolve_retail_bounds(
            config, date_start=date_start, date_end=date_end
        )
        self.id_starts = {key: int(value) for key, value in (id_starts or {}).items()}
        self.existing_dims = {
            key: value.copy(deep=True) for key, value in (existing_dims or {}).items()
        }
    ...
    def build_all_tables(self, graph: SchemaGraph) -> dict[str, pd.DataFrame]:
        ...
        if "dim_customers" in self.existing_dims:
            dim_customers = (
                self.existing_dims["dim_customers"]
                .copy(deep=True)
        ...
        customer_discount_propensities = self._customer_discount_propensities(
            dim_customers
        )

        if "dim_products" in self.existing_dims:
            dim_products = (
                self.existing_dims["dim_products"]
                .copy(deep=True)
                .reset_index(drop=True)
            )
        else:
        ...
        if "dim_stores" in self.existing_dims:
            dim_stores = (
                self.existing_dims["dim_stores"].copy(deep=True).reset_index(drop=True)
            )
        else:
            dim_stores = self._build_dim_stores(counts["dim_stores"], dim_date)

        dim_promotions, promo_lookup_by_date = self._build_dim_promotions(
            counts["dim_promotions"], dim_date
        )
        fact_orders, fact_order_items, fact_payments, bridge = (
            self._build_orders_and_related(
                counts["fact_orders"],
    ...
    def _build_dim_date(self, row_count: int) -> pd.DataFrame:
        if (
            self.config.simulation_start is not None
            or self.config.simulation_end is not None
        ):
            expected_days = (self.range_end.date() - self.range_start.date()).days + 1
            if row_count != expected_days:
                row_count = expected_days
            dates = pd.date_range(
                start=self.range_start.date(), end=self.range_end.date(), freq="D"
            )
        else:
            dates = pd.date_range(
                start=self.range_start.date(), periods=row_count, freq="D"
            )
        return pd.DataFrame(
            {
    ...
    def _build_dim_customers(
        self, row_count: int, dim_date: pd.DataFrame
    ) -> pd.DataFrame:
        max_customer_dt = (
            pd.Timestamp(dim_date["full_date"].max())
            .to_pydatetime()
        ...
        created_at = pd.to_datetime(
            [
                datetime(2021, 1, 1)
                + timedelta(
                    days=int(offset), seconds=int(self.rng.integers(0, 24 * 60 * 60))
                )
        ...
        return pd.DataFrame(
            {
                ...
                "phone": [
                    self.faker.phone_number() if self.rng.random() > 0.09 else None
    ...
    def _customer_propensity_weights(self, dim_customers: pd.DataFrame) -> np.ndarray:
        ...
        weights = np.array(
            [
                float(
                    self.rng.gamma(
                        shape=_SEGMENT_ORDER_WEIGHT_SHAPES.get(segment, 2.0), scale=1.0
                    )
        ...
        if "lifetime_value" in dim_customers.columns:
            ltv = (
                pd.to_numeric(dim_customers["lifetime_value"], errors="coerce")
                .fillna(0.0)
    ...
    def _build_dim_products(self, row_count: int) -> pd.DataFrame:
        categories = weighted_choice(
            list(_CATEGORIES.keys()), _CATEGORY_W, row_count, self.rng
        ).tolist()
        subcategories = [
            str(self.rng.choice(_CATEGORIES[category])) for category in categories
        ]
        cost_price = np.round(
            bounded_lognormal(3.25, 0.75, 2.0, 750.0, row_count, self.rng), 2
        )
        if self.config.discount_variation:
            margin_pct = _ENABLED_MARGIN_MIN + (
                self.rng.beta(2.4, 2.4, row_count) * _ENABLED_MARGIN_RANGE
            )
            list_price = np.round(cost_price / np.clip(1 - margin_pct, 0.08, None), 2)
        else:
            list_price = np.round(
                cost_price * (1 + self.rng.uniform(0.18, 1.10, row_count)), 2
            )
        ...
        return pd.DataFrame(
            {
                ...
                "name": [
                    f"{self.faker.color_name()} {subcategory}"
                    for subcategory in subcategories
                ],
                "category": categories,
                "subcategory": subcategories,
                "brand": [
                    self.faker.company().split()[0]
    ...
    def _customer_discount_propensities(
        ...
        for customer_id in sorted(segment_lookup):
            ...
            lookup[customer_id] = sample_discount_propensity(
                _discount_tier_for_segment(segment), self.discount_rng
            )
        return lookup
    ...
    def _build_dim_stores(self, row_count: int, dim_date: pd.DataFrame) -> pd.DataFrame:
        ...
        store_types = weighted_choice(
            _STORE_TYPES, _STORE_TYPE_W, row_count, self.rng
        ).tolist()
        ...
        return pd.DataFrame(
            {
                ...
                "store_name": [
                    f"{cities[idx]} {store_types[idx].replace('_', ' ').title()}"
                    for idx in range(row_count)
                ],
                ...
                "region": [
                    f"Region-{int(self.rng.integers(1, 9))}" for _ in range(row_count)
                ],
                "opened_date": date_range_samples(
                    date(2012, 1, 1), max_open_date, row_count, self.rng
                ),
    ...
    def _build_dim_promotions(
        ...
        promo_types = weighted_choice(
            _PROMO_TYPES, _PROMO_W, row_count, self.rng
        ).tolist()
        valid_from = date_range_samples(
            dim_date["full_date"].min(),
        ...
        for promo_type, vf in zip(promo_types, valid_from):
            valid_to.append(
                min(
                    vf + timedelta(days=int(self.rng.integers(10, 91))),
                    dim_date["full_date"].max(),
                )
            )
        ...
        df = pd.DataFrame(
            {
                ...
                "promo_name": [
                    f"{str(self.rng.choice(['SAVE', 'FLASH', 'VIP', 'LOYALTY', 'WEEKEND']))}-{start_id + idx:04d}"
                    for idx in range(row_count)
                ],
                ...
                "min_order_value": np.where(
                    self.rng.random(row_count) > 0.45,
                    np.round(self.rng.uniform(20, 180, row_count), 2),
                    None,
                ),
    ...
    def _build_orders_and_related(
        ...
        payment_ids = make_id_list(
            "payment_id", self._id_start("payment_id"), order_count
        )
        customer_idx = self.rng.choice(
            len(dim_customers), size=order_count, p=customer_weights
        )
        store_idx = self.rng.integers(0, len(dim_stores), size=order_count)
        statuses = weighted_choice(
            _ORDER_STATUSES, _ORDER_STATUS_W, order_count, self.rng
        ).tolist()
        channels = weighted_choice(
            _CHANNELS, _CHANNEL_W, order_count, self.rng
        ).tolist()
        line_counts = distribute_counts(
            item_count, order_count, minimum=1, rng=self.rng
        )
        ...
        product_weights = np.clip(
            dim_products["list_price"].to_numpy(dtype=float), 5.0, 300.0
        )
        product_weights = (1 / product_weights) ** 0.3
        product_weights = product_weights / product_weights.sum()
        product_weight_lookup = dim_products.set_index("product_id")[
            "weight_kg"
        ].to_dict()
        ...
        for order_pos, order_id in enumerate(order_ids):
            ...
            qtys = self.rng.choice(
                [1, 2, 3, 4, 5], size=line_count, p=[0.44, 0.27, 0.16, 0.08, 0.05]
            )
            if self.config.discount_variation and self.discount_rng is not None:
                customer_propensity = customer_discount_propensities.get(
                    str(customer["customer_id"]), 0.22
                )
                discounts = np.array(
                    [
                        round(
                            sample_discount(customer_propensity, self.discount_rng), 4
                        )
            ...
            for line_idx in range(line_count):
                ...
                line_total = round(
                    float(qtys[line_idx])
                    * unit_price
                    * (1 - float(discounts[line_idx])),
                    2,
                )
                subtotal += line_total
                total_weight += float(product_weight_lookup[product_id]) * int(
                    qtys[line_idx]
                )
                item_records.append(
                    {
            ...
            selected_promos = self._select_promotions(
                date_id, subtotal, promo_lookup_by_date
            )
            discount_amt = 0.0
            free_shipping = False
            ...
            shipping_amt = self._shipping_amount(
                statuses[order_pos], subtotal, total_weight, free_shipping
            )
            order_total = round(max(subtotal - discount_amt + shipping_amt, 0.0), 2)
            shipped_at, delivered_at = self._order_timestamps(
                statuses[order_pos], created_at
            )
            ...
            payment_records.append(
                {
                    ...
                    "order_id": order_id,
                    "method": str(
                        weighted_choice(_PAYMENT_METHODS, _PAYMENT_W, 1, self.rng)[0]
                    ),
                    "paid_at": paid_at,
                    "amount": amount,
                    "currency": "USD",
        ...
        while len(bridge_pairs) < bridge_target and not fact_orders.empty:
            sampled_order = fact_orders.iloc[
                int(self.rng.integers(0, len(fact_orders)))
            ]
            promo_options = promo_lookup_by_date.get(int(sampled_order["date_id"]), [])
            if not promo_options:
                break
            promo_id = str(
                promo_options[int(self.rng.integers(0, len(promo_options)))]["promo_id"]
            )
        ...
        return (
            ...
            pd.DataFrame(
                {
                    "bridge_id": make_id_list(
                        "bridge_id", self._id_start("bridge_id"), len(bridge_pairs)
                    ),
    ...
    def _date_weights(self, date_frame: pd.DataFrame) -> np.ndarray:
        ...
        weights *= np.where(
            date_frame["day_of_week"].to_numpy(dtype=int) >= 5, 1.18, 1.0
        )
        day_numbers = date_frame["full_date"].dt.day.to_numpy(dtype=int)
        weights *= np.where(
            np.isin(day_numbers, [1, 2, 3, 14, 15, 16, 30, 31]), 1.12, 1.0
        )
        weights *= np.where((day_numbers >= 25) & (day_numbers <= 28), 1.05, 1.0)
        return weights / weights.sum()

    def _sample_order_datetime(
        self, lower_bound: datetime, date_frame: pd.DataFrame
    ) -> datetime:
        eligible = date_frame.loc[
            date_frame["full_date"] >= pd.Timestamp(lower_bound.date())
        ].copy()
        if eligible.empty:
            return lower_bound
    ...
    def _apply_customer_metrics(
        self, dim_customers: pd.DataFrame, fact_orders: pd.DataFrame
    ) -> pd.DataFrame:
        dim_customers = dim_customers.copy(deep=True)
        base_ltv = pd.to_numeric(
            dim_customers.get("lifetime_value", 0.0), errors="coerce"
        ).fillna(0.0)
        customer_ltv = (
        ...
        existing_active = pd.Series(
            dim_customers.get("is_active", False), index=dim_customers.index
        ).astype(bool)
        new_activity = (
            dim_customers["customer_id"]
            .map(customer_recent)
            .apply(lambda ts: pd.Timestamp(ts) >= cutoff if pd.notna(ts) else None)
        )
        dim_customers["is_active"] = new_activity.where(
            new_activity.notna(), existing_active
        ).astype(bool)
    ...
    def _select_promotions(
        ...
        valid_promos = [
            promo
            for promo in promo_lookup_by_date.get(date_id, [])
            if promo["min_order_value"] is None
            or subtotal >= float(promo["min_order_value"])
        ]
        ...
        stackables = [
            ...
            if promo["promo_id"] != selected[0]["promo_id"] and promo["is_stackable"]
        ]
        if stackables and self.rng.random() < 0.08:
            selected.append(stackables[int(self.rng.integers(0, len(stackables)))])
        return selected

    def _shipping_amount(
        self, status: str, subtotal: float, total_weight: float, free_shipping: bool
    ) -> float:
    ...
    def _order_timestamps(
        self, status: str, created_at: datetime
