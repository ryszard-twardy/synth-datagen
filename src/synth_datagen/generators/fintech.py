...
from ..utils import COUNTRY_CURRENCIES, COUNTRY_NAMES, COUNTRY_WEIGHTS, add_months, bounded_lognormal, date_range_samples, datetime_range_samples, distribute_counts, weighted_choice
...
_MERCHANT_CATS = ["retail", "food_beverage", "travel", "utilities", "healthcare", "entertainment", "online_services", "other"]
...
class FintechGenerator(BaseScenarioGenerator):
    ...
    def get_raw_schema(self) -> tuple[list[TableConfig], list[RelationConfig]]:
        ...
        tables = [
            TableConfig(
                ...
                columns=[
                    ...
                    ColumnConfig(name="first_name", dtype=DType.VARCHAR, nullable=False),
                    ...
                    ColumnConfig(name="kyc_status", dtype=DType.VARCHAR, nullable=False),
            ...
            TableConfig(
                ...
                columns=[
                    ColumnConfig(name="account_id", dtype=DType.VARCHAR, nullable=False, unique=True),
                    ColumnConfig(name="customer_id", dtype=DType.VARCHAR, nullable=False),
                    ColumnConfig(name="account_type", dtype=DType.VARCHAR, nullable=False),
                    ...
                    ColumnConfig(name="opened_at", dtype=DType.TIMESTAMP, nullable=False),
                    ColumnConfig(name="closed_at", dtype=DType.TIMESTAMP, nullable=True),
                    ColumnConfig(name="is_active", dtype=DType.BOOLEAN, nullable=False),
                ],
            ),
            TableConfig(
                ...
                columns=[
                    ColumnConfig(name="merchant_id", dtype=DType.VARCHAR, nullable=False, unique=True),
                    ColumnConfig(name="merchant_name", dtype=DType.VARCHAR, nullable=False),
            ...
            TableConfig(
                ...
                columns=[
                    ColumnConfig(name="transaction_id", dtype=DType.VARCHAR, nullable=False, unique=True),
                    ColumnConfig(name="account_id", dtype=DType.VARCHAR, nullable=False),
                    ColumnConfig(name="merchant_id", dtype=DType.VARCHAR, nullable=True),
                    ColumnConfig(name="tx_type", dtype=DType.VARCHAR, nullable=False),
                    ColumnConfig(name="amount", dtype=DType.DECIMAL, nullable=False),
                    ColumnConfig(name="currency", dtype=DType.VARCHAR, nullable=False),
                    ColumnConfig(name="balance_after", dtype=DType.DECIMAL, nullable=False),
            ...
            TableConfig(
                ...
                pk_column="card_id",
                columns=[
                    ColumnConfig(name="card_id", dtype=DType.VARCHAR, nullable=False, unique=True),
                    ColumnConfig(name="account_id", dtype=DType.VARCHAR, nullable=False),
                    ...
                    ColumnConfig(name="last4", dtype=DType.VARCHAR, nullable=False, max_length=4),
                    ...
                    ColumnConfig(name="is_active", dtype=DType.BOOLEAN, nullable=False),
                    ColumnConfig(name="spend_limit", dtype=DType.DECIMAL, nullable=True),
                ],
            ),
            TableConfig(
                name="loans",
                ...
                pk_column="loan_id",
                columns=[
                    ColumnConfig(name="loan_id", dtype=DType.VARCHAR, nullable=False, unique=True),
                    ColumnConfig(name="customer_id", dtype=DType.VARCHAR, nullable=False),
                    ...
                    ColumnConfig(name="interest_rate", dtype=DType.DECIMAL, nullable=False),
                    ColumnConfig(name="term_months", dtype=DType.INT, nullable=False),
                    ColumnConfig(name="monthly_payment", dtype=DType.DECIMAL, nullable=False),
                    ColumnConfig(name="outstanding", dtype=DType.DECIMAL, nullable=False),
                    ColumnConfig(name="status", dtype=DType.VARCHAR, nullable=False),
                    ColumnConfig(name="disbursed_at", dtype=DType.TIMESTAMP, nullable=False),
            ...
            TableConfig(
                ...
                pk_column="lp_id",
                columns=[
                    ColumnConfig(name="lp_id", dtype=DType.VARCHAR, nullable=False, unique=True),
        ...
        relations = [
            RelationConfig(source_table="accounts", source_column="customer_id", target_table="customers", target_column="customer_id"),
            RelationConfig(source_table="transactions", source_column="account_id", target_table="accounts", target_column="account_id"),
            RelationConfig(source_table="transactions", source_column="merchant_id", target_table="merchants", target_column="merchant_id"),
            RelationConfig(source_table="cards", source_column="account_id", target_table="accounts", target_column="account_id"),
            RelationConfig(source_table="loans", source_column="customer_id", target_table="customers", target_column="customer_id"),
            RelationConfig(source_table="loan_payments", source_column="loan_id", target_table="loans", target_column="loan_id"),
        ]
    ...
    def _build_all_tables(self, graph: SchemaGraph) -> dict[str, pd.DataFrame]:
        ...
        transactions, account_balances = self._build_transactions(counts["transactions"], accounts, merchants)
        accounts["balance"] = accounts["account_id"].map(account_balances).fillna(accounts["balance"]).round(2)
    ...
    def _build_customers(self, row_count: int) -> pd.DataFrame:
        ...
        created_at = pd.to_datetime(datetime_range_samples(_SIM_START, datetime(2025, 9, 30), row_count, self.rng))
        ...
        return pd.DataFrame(
            {
                ...
                "email": [f"{first_names[idx].lower()}.{last_names[idx].lower()}.{idx + 1:05d}@mail.com" for idx in range(row_count)],
                "phone": [self.faker.phone_number() if self.rng.random() > 0.10 else None for _ in range(row_count)],
                "dob": date_range_samples(date(1950, 1, 1), date(2003, 12, 31), row_count, self.rng),
                "country": countries,
                "city": [self.faker.city() for _ in range(row_count)],
                "credit_score": np.round(self.rng.normal(690, 85, row_count)).clip(300, 850).astype(int),
                "kyc_status": weighted_choice(["verified", "pending", "rejected", "not_started"], [0.78, 0.12, 0.05, 0.05], row_count, self.rng).tolist(),
    ...
    def _build_accounts(self, row_count: int, customers: pd.DataFrame) -> pd.DataFrame:
        ...
        for cust_idx, acct_count in enumerate(counts):
            ...
            for _ in range(int(acct_count)):
                account_type = str(weighted_choice(_ACCOUNT_TYPES, _ACCOUNT_W, 1, self.rng)[0])
                opened_at = datetime_range_samples(customer_created, _AS_OF - timedelta(days=30), 1, self.rng)[0]
                is_active = bool(self.rng.random() > 0.14)
                closed_at = None
                if not is_active:
                    closed_at = datetime_range_samples(opened_at + timedelta(days=30), _AS_OF, 1, self.rng)[0]
                records.append(
                    {
                        ...
                        "balance": round(float(bounded_lognormal(6.0, 1.0, 100.0, 250_000.0, 1, self.rng)[0]), 2),
                        "opened_at": opened_at,
                        "closed_at": closed_at,
                        "is_active": is_active,
                    }
        ...
        return pd.DataFrame(
            {
                ...
                "category": weighted_choice(_MERCHANT_CATS, _MERCHANT_W, row_count, self.rng).tolist(),
    ...
    def _build_transactions(
        ...
        tx_counts = distribute_counts(row_count, len(accounts), minimum=0, rng=self.rng, weights=weights)
        ...
        for idx, tx_count in enumerate(tx_counts):
            ...
            end = pd.Timestamp(account["closed_at"]).to_pydatetime() if pd.notna(account["closed_at"]) else _AS_OF
            if start >= end:
                end = start + timedelta(days=1)
            balance = float(account["balance"])
            timestamps = sorted(datetime_range_samples(start, end, int(tx_count), self.rng).tolist()) if tx_count else []
            for ts in timestamps:
                tx_type = str(weighted_choice(_TX_TYPES, _TX_WEIGHTS, 1, self.rng)[0])
                raw_amount = round(float(bounded_lognormal(3.3, 1.0, 1.0, 25_000.0, 1, self.rng)[0]), 2)
                ...
                balance_floor = -50_000.0 if account["account_type"] == "credit" else -500.0
                ...
                records.append(
                    {
                        ...
                        "description": self.faker.sentence(nb_words=5) if self.rng.random() > 0.25 else None,
                        "status": str(weighted_choice(["completed", "pending"], [0.92, 0.08], 1, self.rng)[0]),
                        "created_at": ts,
                    }
                )
                tx_seq += 1
            balances[str(account["account_id"])] = round(balance, 2)
        transactions = pd.DataFrame(records).sort_values(["account_id", "created_at", "transaction_id"]).reset_index(drop=True)
        return transactions, balances

    def _build_cards(self, row_count: int, accounts: pd.DataFrame) -> pd.DataFrame:
        counts = distribute_counts(row_count, len(accounts), minimum=0, rng=self.rng, weights=np.where(accounts["account_type"].eq("credit"), 1.6, 1.0))
        records: list[dict[str, object]] = []
        seq = 1
        for idx, count in enumerate(counts):
            account = accounts.iloc[idx]
            issue_start = pd.Timestamp(account["opened_at"]).date()
            issue_end = min((_AS_OF - timedelta(days=90)).date(), pd.Timestamp(account["closed_at"]).date() if pd.notna(account["closed_at"]) else _AS_OF.date())
            if issue_start > issue_end:
                issue_end = issue_start
            issue_dates = date_range_samples(issue_start, issue_end, int(count), self.rng)
            for issue_date in issue_dates:
                expiry_date = issue_date.replace(year=issue_date.year + int(self.rng.integers(3, 6)))
                is_active = bool(expiry_date >= _AS_OF.date() and account["is_active"] and self.rng.random() > 0.10)
                ...
                records.append(
                    {
                        ...
                        "account_id": str(account["account_id"]),
                        "card_type": card_type,
                        "network": str(weighted_choice(_CARD_NETWORKS, _CARD_NW, 1, self.rng)[0]),
                        "last4": f"{int(self.rng.integers(0, 10_000)):04d}",
                        "issue_date": issue_date,
                        "expiry_date": expiry_date,
                        "is_active": is_active,
                        "spend_limit": round(float(self.rng.uniform(500, 30_000)), 2) if card_type == "credit" else None,
                    }
                )
                seq += 1
        return pd.DataFrame(records)
    ...
    def _build_loans(self, row_count: int, customers: pd.DataFrame) -> pd.DataFrame:
        chosen_customers = self.rng.choice(len(customers), size=row_count, replace=row_count > len(customers))
        records: list[dict[str, object]] = []
        for idx, cust_pos in enumerate(chosen_customers, start=1):
            customer = customers.iloc[int(cust_pos)]
            disbursed_at = datetime_range_samples(pd.Timestamp(customer["created_at"]).to_pydatetime(), _AS_OF - timedelta(days=30), 1, self.rng)[0]
            loan_type = str(weighted_choice(_LOAN_TYPES, _LOAN_W, 1, self.rng)[0])
            principal = round(float(bounded_lognormal(9.0, 1.0, 1_000.0, 900_000.0, 1, self.rng)[0]), 2)
    ...
    def _build_loan_payments(self, row_count: int, loans: pd.DataFrame) -> pd.DataFrame:
        ...
        for loan in loans.itertuples(index=False):
            ...
            for installment in range(total_installments):
                due_dt = add_months(pd.Timestamp(loan.disbursed_at).to_pydatetime(), installment + 1)
                if due_dt.date() > loan.due_date:
                    break
                status = str(weighted_choice(_PAYMENT_STATUSES, _PAYMENT_SW, 1, self.rng)[0])
                amount = round(float(self.rng.uniform(max(1.0, loan.monthly_payment * 0.75), loan.monthly_payment)), 2)
                if status in {"failed", "reversed"}:
                    amount = round(float(self.rng.uniform(0.0, loan.monthly_payment * 0.15)), 2)
                paid_at = datetime_range_samples(pd.Timestamp(loan.disbursed_at).to_pydatetime(), due_dt, 1, self.rng)[0]
                candidates.append(
                    {
                        ...
                        "paid_at": paid_at,
                        "status": status,
                        "penalty": round(float(self.rng.uniform(10, 200)), 2) if paid_at > due_dt and self.rng.random() < 0.25 else 0.0,
                    }
                )
