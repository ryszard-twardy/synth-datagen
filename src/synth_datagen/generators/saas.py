...
from ..utils import add_months, bounded_lognormal, datetime_range_samples, stable_uuid, weighted_choice
from .base import BaseScenarioGenerator

_PLANS = ["free", "starter", "growth", "professional", "enterprise"]
_PLAN_W = [0.24, 0.28, 0.24, 0.16, 0.08]
_PLAN_PRICE = {"free": 0.0, "starter": 29.0, "growth": 79.0, "professional": 199.0, "enterprise": 499.0}
...
_EVENT_TYPES = ["login", "page_view", "feature_click", "export", "invite_sent", "api_call", "settings_change", "logout"]
...
class SaasGenerator(BaseScenarioGenerator):
    ...
    def get_raw_schema(self) -> tuple[list[TableConfig], list[RelationConfig]]:
        ...
        tables = [
            TableConfig(
                ...
                columns=[
                    ...
                    ColumnConfig(name="company_name", dtype=DType.VARCHAR, nullable=False, max_length=160),
                    ColumnConfig(name="domain", dtype=DType.VARCHAR, nullable=False, unique=True),
                    ColumnConfig(name="industry", dtype=DType.VARCHAR, nullable=False, max_length=80),
            ...
            TableConfig(
                ...
                pk_column="user_id",
                columns=[
                    ColumnConfig(name="user_id", dtype=DType.VARCHAR, nullable=False, unique=True),
                    ...
                    ColumnConfig(name="created_at", dtype=DType.TIMESTAMP, nullable=False),
                    ColumnConfig(name="last_login", dtype=DType.TIMESTAMP, nullable=True),
            ...
            TableConfig(
                ...
                pk_column="sub_id",
                columns=[
                    ColumnConfig(name="sub_id", dtype=DType.VARCHAR, nullable=False, unique=True),
                    ColumnConfig(name="account_id", dtype=DType.VARCHAR, nullable=False),
                    ...
                    ColumnConfig(name="started_at", dtype=DType.TIMESTAMP, nullable=False),
                    ColumnConfig(name="ended_at", dtype=DType.TIMESTAMP, nullable=True),
                    ColumnConfig(name="billing_cycle", dtype=DType.VARCHAR, nullable=False),
                ],
            ),
            TableConfig(
                name="invoices",
                ...
                columns=[
                    ColumnConfig(name="invoice_id", dtype=DType.VARCHAR, nullable=False, unique=True),
                    ColumnConfig(name="account_id", dtype=DType.VARCHAR, nullable=False),
                    ...
                    ColumnConfig(name="status", dtype=DType.VARCHAR, nullable=False),
                    ColumnConfig(name="issued_at", dtype=DType.TIMESTAMP, nullable=False),
            ...
            TableConfig(
                ...
                columns=[
                    ColumnConfig(name="feature_id", dtype=DType.VARCHAR, nullable=False, unique=True),
                    ColumnConfig(name="name", dtype=DType.VARCHAR, nullable=False, unique=True),
            ...
            TableConfig(
                ...
                pk_column="usage_id",
                columns=[
                    ColumnConfig(name="usage_id", dtype=DType.VARCHAR, nullable=False, unique=True),
                    ColumnConfig(name="user_id", dtype=DType.VARCHAR, nullable=False),
                    ColumnConfig(name="feature_id", dtype=DType.VARCHAR, nullable=False),
            ...
            TableConfig(
                ...
                pk_column="event_id",
                columns=[
                    ColumnConfig(name="event_id", dtype=DType.VARCHAR, nullable=False, unique=True),
                    ColumnConfig(name="user_id", dtype=DType.VARCHAR, nullable=False),
                    ColumnConfig(name="event_type", dtype=DType.VARCHAR, nullable=False),
                    ColumnConfig(name="occurred_at", dtype=DType.TIMESTAMP, nullable=False),
        ...
        relations = [
            RelationConfig(source_table="users", source_column="account_id", target_table="accounts", target_column="account_id"),
            RelationConfig(source_table="subscriptions", source_column="account_id", target_table="accounts", target_column="account_id"),
            RelationConfig(source_table="invoices", source_column="account_id", target_table="accounts", target_column="account_id"),
            RelationConfig(source_table="invoices", source_column="sub_id", target_table="subscriptions", target_column="sub_id"),
            RelationConfig(source_table="feature_usage", source_column="user_id", target_table="users", target_column="user_id"),
            RelationConfig(source_table="feature_usage", source_column="feature_id", target_table="features", target_column="feature_id"),
            RelationConfig(source_table="events", source_column="user_id", target_table="users", target_column="user_id"),
        ]
    ...
    def _build_all_tables(self, graph: SchemaGraph) -> dict[str, pd.DataFrame]:
        ...
        account_mrr = (
            subscriptions.loc[subscriptions["status"].isin(["active", "past_due", "trialing", "paused"])]
        ...
        feature_usage = self._build_feature_usage(counts["feature_usage"], users, features, subscriptions)
    ...
    def _build_accounts(self, row_count: int) -> pd.DataFrame:
        industries = ["SaaS", "Fintech", "Healthcare", "Retail", "Media", "Education", "Logistics", "Manufacturing"]
        company_names = [self.faker.company() for _ in range(row_count)]
        return pd.DataFrame(
            {
                "account_id": make_id_list("account_id", 1, row_count),
                "company_name": company_names,
                "domain": [f"{self.faker.domain_word()}-{idx + 1}.io" for idx in range(row_count)],
                "industry": [str(self.rng.choice(industries)) for _ in range(row_count)],
                "employee_count": np.round(bounded_lognormal(3.8, 1.1, 5, 5_000, row_count, self.rng)).astype(int),
                "country": weighted_choice(
                    ["United States", "United Kingdom", "Germany", "Canada", "Australia", "Netherlands"],
    ...
    def _build_subscriptions(self, row_count: int, accounts: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx, sub_count in enumerate(counts):
            ...
            timeline_start = pd.Timestamp(account["created_at"]).to_pydatetime() + timedelta(days=int(self.rng.integers(0, 45)))
            for sub_idx in range(int(sub_count)):
                ...
                billing_cycle = str(weighted_choice(["monthly", "annual"], [0.72, 0.28], 1, self.rng)[0])
                seats_factor = max(1.0, float(account["employee_count"]) / 80.0)
                mrr = round(_PLAN_PRICE[plan] * (1.0 if plan == "free" else min(seats_factor, 20.0)), 2)
                if sub_idx < sub_count - 1:
                    duration_months = int(self.rng.integers(6, 20))
                    ended_at = min(add_months(timeline_start, duration_months), _AS_OF - timedelta(days=30))
                    status = "cancelled"
                else:
                    status = str(weighted_choice(_SUB_STATUSES, _SUB_STATUS_W, 1, self.rng)[0])
                    ended_at = None
                    if status == "cancelled":
                        ended_at = min(add_months(timeline_start, int(self.rng.integers(2, 18))), _AS_OF)
                records.append(
                    {
                ...
                seq += 1
                timeline_start = (ended_at or _AS_OF) + timedelta(days=int(self.rng.integers(3, 45)))
    ...
    def _build_users(self, row_count: int, accounts: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx in range(row_count):
            ...
            created_at = datetime_range_samples(pd.Timestamp(account["created_at"]).to_pydatetime(), _AS_OF, 1, self.rng)[0]
    ...
    def _build_features(self, row_count: int) -> pd.DataFrame:
        base_names = [
            "Dashboard", "Reports", "API Access", "Export CSV", "Webhooks", "SSO", "Custom Roles", "Audit Log",
            "2FA", "White Label", "Data Import", "Integrations", "Custom Domain", "Priority Support", "SLA Guarantee",
            "Analytics", "Collaboration", "Version History", "Backups", "Advanced Search", "Bulk Actions", "Team Management",
            "Custom Alerts", "Mobile App", "CLI Tool", "SDK Access", "Sandbox Environment", "Usage Reports", "Billing Portal",
            "Multi Currency",
        ]
        ...
        return pd.DataFrame(
            {
                ...
                "plan_min": weighted_choice(_PLANS, [0.20, 0.24, 0.24, 0.20, 0.12], row_count, self.rng).tolist(),
                "is_beta": self.rng.random(row_count) < 0.12,
            }
        )

    def _build_invoices(self, row_count: int, subscriptions: pd.DataFrame) -> pd.DataFrame:
        candidates: list[dict[str, object]] = []
        for sub in subscriptions.itertuples(index=False):
            started_at = pd.Timestamp(sub.started_at).to_pydatetime()
            ended_at = pd.Timestamp(sub.ended_at).to_pydatetime() if pd.notna(sub.ended_at) else _AS_OF
            ...
            while issued_at <= ended_at:
                ...
                if due_at < _AS_OF - timedelta(days=45):
                    status = str(weighted_choice(_INV_STATUSES, _INV_STATUS_W, 1, self.rng)[0])
                else:
                    status = str(weighted_choice(["paid", "pending", "overdue"], [0.72, 0.22, 0.06], 1, self.rng)[0])
                paid_at = None
                if status == "paid":
                    paid_at = datetime_range_samples(issued_at, min(due_at, _AS_OF), 1, self.rng)[0]
                candidates.append(
                    {
        ...
        if len(candidates) >= row_count:
            ...
            while len(rows) < row_count and candidates:
                ...
                seed["issued_at"] = min(pd.Timestamp(seed["issued_at"]).to_pydatetime() + timedelta(days=7), _AS_OF)
                seed["due_at"] = min(pd.Timestamp(seed["due_at"]).to_pydatetime() + timedelta(days=7), _AS_OF + timedelta(days=30))
                seed["status"] = "pending"
                seed["paid_at"] = None
                rows.append(seed)
    ...
    def _active_plan_intervals(self, subscriptions: pd.DataFrame) -> dict[str, list[tuple[datetime, datetime, str]]]:
        intervals: dict[str, list[tuple[datetime, datetime, str]]] = defaultdict(list)
        for sub in subscriptions.itertuples(index=False):
            start = pd.Timestamp(sub.started_at).to_pydatetime()
            end = pd.Timestamp(sub.ended_at).to_pydatetime() if pd.notna(sub.ended_at) else _AS_OF
    ...
    def _build_feature_usage(
        ...
        feature_by_rank = {
            rank: features.loc[features["plan_min"].map(_PLAN_RANK) <= rank, "feature_id"].tolist()
        ...
        for idx, user_pos in enumerate(sampled_users, start=1):
            ...
            account_intervals = [
                interval for interval in intervals.get(str(user["account_id"]), [])
                if interval[1] >= user_created
            ]
            if account_intervals:
                interval = account_intervals[int(self.rng.integers(0, len(account_intervals)))]
            ...
            records.append(
                {
                    ...
                    "used_at": used_at,
                    "duration_s": int(self.rng.integers(5, 3_601)) if self.rng.random() > 0.08 else None,
                }
            )
        return pd.DataFrame(records)
    ...
    def _build_events(self, row_count: int, users: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx, user_pos in enumerate(sampled_users, start=1):
            user = users.iloc[int(user_pos)]
            occurred_at = datetime_range_samples(pd.Timestamp(user["created_at"]).to_pydatetime(), _AS_OF, 1, self.rng)[0]
            records.append(
                {
                    "event_id": make_id_list("event_id", idx, 1)[0],
                    "user_id": str(user["user_id"]),
                    "event_type": str(weighted_choice(_EVENT_TYPES, _EVENT_W, 1, self.rng)[0]),
                    "occurred_at": occurred_at,
                    "session_id": stable_uuid(f"session-{user['user_id']}-{occurred_at.isoformat()}") if self.rng.random() > 0.05 else None,
                    "ip_address": self.faker.ipv4() if self.rng.random() > 0.03 else None,
                }
            )
        return pd.DataFrame(records)
