...
class LogisticsGenerator(BaseScenarioGenerator):
    ...
    def get_raw_schema(self) -> tuple[list[TableConfig], list[RelationConfig]]:
        ...
        tables = [
            ...
            TableConfig(
                ...
                columns=[
                    ...
                    ColumnConfig(
                        name="supplier_id", dtype=DType.VARCHAR, nullable=False
                    ),
                    ...
                    ColumnConfig(
                        name="is_hazardous", dtype=DType.BOOLEAN, nullable=False
                    ),
                ],
            ),
            TableConfig(
                ...
                columns=[
                    ColumnConfig(
                        name="inv_id", dtype=DType.VARCHAR, nullable=False, unique=True
                    ),
                    ColumnConfig(
                        name="warehouse_id", dtype=DType.VARCHAR, nullable=False
                    ),
                    ColumnConfig(
                        name="product_id", dtype=DType.VARCHAR, nullable=False
                    ),
                    ...
                    ColumnConfig(
                        name="last_updated", dtype=DType.TIMESTAMP, nullable=False
                    ),
            ...
            TableConfig(
                ...
                columns=[
                    ...
                    ColumnConfig(
                        name="carrier_id", dtype=DType.VARCHAR, nullable=False
                    ),
                    ColumnConfig(name="origin_wh", dtype=DType.VARCHAR, nullable=False),
                    ColumnConfig(
                        name="dest_country", dtype=DType.VARCHAR, nullable=False
                    ),
                    ColumnConfig(name="status", dtype=DType.VARCHAR, nullable=False),
                    ColumnConfig(
                        name="transport_mode", dtype=DType.VARCHAR, nullable=False
                    ),
                    ColumnConfig(name="incoterm", dtype=DType.VARCHAR, nullable=True),
                    ColumnConfig(
                        name="shipped_at", dtype=DType.TIMESTAMP, nullable=False
                    ),
                    ColumnConfig(
                        name="estimated_at", dtype=DType.TIMESTAMP, nullable=False
                    ),
                    ColumnConfig(
                        name="delivered_at", dtype=DType.TIMESTAMP, nullable=True
                    ),
                    ColumnConfig(
                        name="freight_cost", dtype=DType.DECIMAL, nullable=False
                    ),
                ],
            ),
            TableConfig(
                ...
                columns=[
                    ColumnConfig(
                        name="si_id", dtype=DType.VARCHAR, nullable=False, unique=True
                    ),
                    ColumnConfig(
                        name="shipment_id", dtype=DType.VARCHAR, nullable=False
                    ),
                    ColumnConfig(
                        name="product_id", dtype=DType.VARCHAR, nullable=False
                    ),
                    ...
                    ColumnConfig(name="unit_cost", dtype=DType.DECIMAL, nullable=False),
                    ColumnConfig(
                        name="line_value", dtype=DType.DECIMAL, nullable=False
                    ),
                ],
            ),
        ]
        relations = [
            ...
            RelationConfig(
                ...
                source_column="carrier_id",
                target_table="carriers",
                target_column="carrier_id",
            ),
            RelationConfig(
                source_table="shipments",
                source_column="origin_wh",
                target_table="warehouses",
                target_column="warehouse_id",
            ),
            RelationConfig(
                source_table="shipment_items",
    ...
    def _build_all_tables(self, graph: SchemaGraph) -> dict[str, pd.DataFrame]:
        ...
        shipments, shipment_items = self._build_shipments_and_items(
            counts["shipments"],
    ...
    def _build_products(self, row_count: int, suppliers: pd.DataFrame) -> pd.DataFrame:
        ...
        unit_cost = np.round(
            bounded_lognormal(3.4, 0.9, 0.5, 4_000.0, row_count, self.rng), 2
        )
        return pd.DataFrame(
            {
                ...
                "name": [
                    f"{self.faker.color_name()} {str(self.rng.choice(categories))} Item"
                ...
                "supplier_id": [
                    str(self.rng.choice(supplier_ids)) for _ in range(row_count)
                ],
    ...
    def _build_inventory(
        self, row_count: int, warehouses: pd.DataFrame, products: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        qty_reserved = np.minimum(
            self.rng.integers(0, 700, size=row_count), qty_on_hand
        )
        return pd.DataFrame(
            {
                ...
                "last_updated": pd.to_datetime(
                    datetime_range_samples(_SIM_START, _AS_OF, row_count, self.rng)
                ),
    ...
    def _build_carriers(self, row_count: int) -> pd.DataFrame:
        transport = weighted_choice(
            _TRANSPORT, _TRANSPORT_W, row_count, self.rng
        ).tolist()
        return pd.DataFrame(
            {
    ...
    def _build_shipments_and_items(
        ...
        line_counts = distribute_counts(
            item_count, shipment_count, minimum=1, rng=self.rng
        )
        ...
        for idx, shipment_id in enumerate(shipment_ids):
            ...
            dest_country = str(
                weighted_choice(COUNTRY_NAMES, COUNTRY_WEIGHTS, 1, self.rng)[0]
            )
            shipped_at = datetime_range_samples(
                _SIM_START, _AS_OF - timedelta(days=1), 1, self.rng
            )[0]
            transit_days = {
                "air": int(self.rng.integers(1, 5)),
                "courier": int(self.rng.integers(1, 4)),
                "road": int(self.rng.integers(2, 12)),
                "rail": int(self.rng.integers(5, 18)),
                "sea": int(self.rng.integers(12, 45)),
            }[carrier_transport]
            estimated_at = shipped_at + timedelta(days=transit_days)
            status = str(weighted_choice(_SHIP_STATUS, _SHIP_W, 1, self.rng)[0])
            delivered_at = None
            if status == "delivered":
                delivered_at = estimated_at + timedelta(
            ...
            chosen_products = self.rng.choice(
                product_ids, size=line_count, replace=line_count > len(product_ids)
            )
            ...
            freight_cost = round(
                max(
                    20.0,
                    8.0
                    + total_weight * 0.12 * mode_factor * distance_factor
                    + line_value_sum * 0.002,
                ),
                2,
            )
            shipment_records.append(
                {
                    ...
                    "incoterm": str(
                        weighted_choice(
                            _INCOTERMS, [0.18, 0.20, 0.18, 0.24, 0.20], 1, self.rng
                        )[0]
                    ),
