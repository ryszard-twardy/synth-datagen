...
def generate(
    ...
    profile_config: Path | None = typer.Option(
        None, "--profile-config", help="YAML profile for monthly-sales generation."
    ),
    ...
    month: str | None = typer.Option(
        None, "--month", help="Convenience monthly range in YYYY-MM format."
    ),
    orders_per_month: int | None = typer.Option(
        None, "--orders-per-month", help="Target orders per month."
    ),
    avg_items_per_order: float = typer.Option(
        2.5, "--avg-items-per-order", help="Average line items per order."
    ),
    layout: str = typer.Option(
        "monthly", "--layout", help="Output layout: monthly | combined | both."
    ),
    include_flat: bool = typer.Option(
        True,
        "--include-flat/--no-flat",
        help="Also export a flat monthly sales extract.",
    ),
    resume_from: Path | None = typer.Option(
        None, "--resume-from", help="Prior combined snapshot to append from."
    ),
    output: Path = typer.Option(
        Path("./out/monthly_sales"), "--output", "-o", help="Output directory."
    ),
    ...
    data_quality: DataQuality = typer.Option(
        ...
        "--data-quality",
        "--dq",
        help="Dirty-but-plausible data quality mode.",
    ),
    export_parquet: bool = typer.Option(
        False,
        "--export-parquet/--no-parquet",
        help="Also export combined parquet files.",
    ),
    export_sqlite: bool = typer.Option(
        False,
        "--export-sqlite/--no-sqlite",
        help="Also export a combined SQLite database.",
    ),
    customers: int | None = typer.Option(
        None, "--customers", help="Optional total customer dimension size."
    ),
    products: int | None = typer.Option(
        None, "--products", help="Optional total product dimension size."
    ),
    stores: int | None = typer.Option(
        None, "--stores", help="Optional total store dimension size."
    ),
    promotions: int | None = typer.Option(
        None, "--promotions", help="Optional total promotion dimension size."
    ),
    prorate_partial_months: bool = typer.Option(
        True,
        "--prorate-partial-months/--no-prorate-partial-months",
        help="Scale first/last partial months by active-day share.",
    ),
) -> None:
    ...
    try:
        (
            ...
            load_monthly_sales_profile,
        ) = _load_monthly_runtime()
    ...
    if profile_config is not None:
        ...
        if orders_per_month is None:
            raise typer.BadParameter(
                "--orders-per-month is required unless --profile-config is used.",
                param_hint="--orders-per-month",
            )
...
def _normalize_layout(value: str) -> str:
    ...
    if normalized not in {"monthly", "combined", "both", "sales-files"}:
        raise typer.BadParameter(
            "Layout must be one of: monthly, combined, both, sales-files.",
            param_hint="--layout",
        )
    return normalized
