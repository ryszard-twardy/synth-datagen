...
class DefectInjector:
    ...
    def apply(self, clean: GeneratedTables) -> GeneratedTables:
        tables = {
            table_name: [batch.copy(deep=True) for batch in batches]
            for table_name, batches in clean.tables.items()
        }
        ...
        dirty = GeneratedTables(
            ...
            hidden_tables={
                ...
                for name, frame in clean.hidden_tables.items()
            },
            metadata={
                "mode": "dirty",
                "seed": self.seed,
    ...
    def _inject_null_company_names(
        self,
        ...
        summary["null_company_names"] = {
            "table": "accounts",
            "actual_count": count,
            "actual_rate": count / max(1, len(accounts)),
        }
    ...
    def _inject_case_duplicate_emails(
        self,
        tables: dict[str, list[pd.DataFrame]],
        summary: dict[str, dict[str, object]],
    ) -> None:
        ...
        if count and len(users) > 1:
            ...
            for target, source in zip(
                target_idx.tolist(), source_idx.tolist(), strict=False
            ):
                ...
                actual += 1
        summary["case_duplicate_emails"] = {
            "table": "users",
            "actual_count": actual,
            "actual_rate": actual / max(1, len(users)),
        }

    def _inject_pre_signup_logins(
        self,
        tables: dict[str, list[pd.DataFrame]],
        summary: dict[str, dict[str, object]],
    ) -> None:
        ...
        if count and len(eligible):
            ...
            for idx in picks.tolist():
                signup = pd.Timestamp(
                    accounts.loc[users.at[idx, "account_id"], "signup_date"]
                ).date()
                offset_days = int(rng.integers(1, 31))
                login_date = signup - timedelta(days=offset_days)
                users.at[idx, "last_login_at"] = datetime.combine(
                    login_date,
                    time(
                        hour=int(rng.integers(5, 21)), minute=int(rng.integers(0, 60))
                    ),
                )
                actual += 1
        summary["pre_signup_logins"] = {
            "table": "users",
            "actual_count": actual,
            "actual_rate": actual / max(1, len(users)),
        }
    ...
    def _inject_plan_typos(
        ...
        if count:
            ...
            for idx in picks.tolist():
                subscriptions.at[idx, "plan_name"] = self._plan_typo(
                    str(subscriptions.at[idx, "plan_name"])
                )
                actual += 1
        summary["plan_name_typos"] = {
            "table": "subscriptions",
            "actual_count": actual,
            "actual_rate": actual / max(1, len(subscriptions)),
        }

    def _inject_negative_monthly_amounts(
        self,
        tables: dict[str, list[pd.DataFrame]],
        summary: dict[str, dict[str, object]],
    ) -> None:
        ...
        if count:
            picks = rng.choice(len(subscriptions), size=count, replace=False)
            subscriptions.loc[picks, "monthly_amount"] = -subscriptions.loc[
                picks, "monthly_amount"
            ].abs()
            actual = len(picks)
        summary["negative_monthly_amounts"] = {
            "table": "subscriptions",
            "actual_count": actual,
            "actual_rate": actual / max(1, len(subscriptions)),
        }

    def _inject_reversed_subscription_dates(
        self,
        tables: dict[str, list[pd.DataFrame]],
        summary: dict[str, dict[str, object]],
    ) -> None:
        ...
        if count and len(eligible):
            ...
            for idx in picks.tolist():
                ...
                subscriptions.at[idx, "end_date"] = start - timedelta(
                    days=int(rng.integers(1, 30))
                )
                actual += 1
        summary["reversed_subscription_dates"] = {
            "table": "subscriptions",
            "actual_count": actual,
            "actual_rate": actual / max(1, len(subscriptions)),
        }

    def _inject_orphan_events(
        self,
        tables: dict[str, list[pd.DataFrame]],
        summary: dict[str, dict[str, object]],
    ) -> None:
        ...
        for batch_index, batch in enumerate(tables["product_events"]):
            batch_count = min(
                len(batch),
                _count_from_rate(len(batch), cfg.rate if cfg.enabled else 0.0),
            )
            if batch_count <= 0:
                continue
            picks = rng.choice(len(batch), size=batch_count, replace=False)
            for offset, idx in enumerate(picks.tolist(), start=1):
                batch.at[idx, "account_id"] = orphan_id(
                    "account_id", batch_index * 1_000_000 + offset
                )
                batch.at[idx, "user_id"] = orphan_id(
                    "user_id", batch_index * 1_000_000 + offset
                )
                actual += 1
        summary["orphaned_product_events"] = {
    ...
    def _inject_future_timestamps(
        ...
        if user_count:
            ...
            if len(eligible):
                picks = rng.choice(
                    eligible, size=min(user_count, len(eligible)), replace=False
                )
                for idx in picks.tolist():
                    users.at[idx, "last_login_at"] = datetime.combine(
                        future_base + timedelta(days=int(rng.integers(0, 45))),
                        time(
        ...
        if inv_count:
            ...
            invoices.loc[picks, "invoice_date"] = [
                future_base + timedelta(days=int(rng.integers(0, 60)))
        ...
        if nps_count:
            ...
            nps.loc[picks, "survey_date"] = [
                future_base + timedelta(days=int(rng.integers(0, 90)))
                for _ in range(nps_count)
            ]
            actual += nps_count
        event_total = 0
        for batch in tables["product_events"]:
            ...
            if batch_count <= 0:
                continue
            picks = rng.choice(len(batch), size=batch_count, replace=False)
            batch.loc[picks, "event_timestamp"] = [
                datetime.combine(
                    future_base + timedelta(days=int(rng.integers(0, 90))),
                    time(
        ...
        summary["future_timestamps"] = {
            "tables": ["users", "invoices", "nps_responses", "product_events"],
            "actual_count": actual,
    ...
    def _format_amount_variant(
        self, amount: float, currency: str, rng: np.random.Generator
    ) -> str:
    ...
    def _inject_mixed_invoice_amount_formats(
        self,
        tables: dict[str, list[pd.DataFrame]],
        summary: dict[str, dict[str, object]],
    ) -> None:
        ...
        if count:
            ...
            for idx in picks.tolist():
                invoices.at[idx, "amount_due"] = self._format_amount_variant(
                    float(invoices.at[idx, "amount_due"]),
                    str(invoices.at[idx, "currency"]),
                    rng,
                )
                invoices.at[idx, "amount_paid"] = self._format_amount_variant(
                    float(invoices.at[idx, "amount_paid"]),
                    str(invoices.at[idx, "currency"]),
                    rng,
                )
                actual += 1
        summary["mixed_invoice_amount_formats"] = {
            "table": "invoices",
            "actual_count": actual,
            "actual_rate": actual / max(1, len(invoices)),
        }

    def _inject_out_of_range_nps_scores(
        self,
        tables: dict[str, list[pd.DataFrame]],
        summary: dict[str, dict[str, object]],
    ) -> None:
        ...
        if count:
            ...
            actual = len(picks)
        summary["out_of_range_nps_scores"] = {
            "table": "nps_responses",
            "actual_count": actual,
            "actual_rate": actual / max(1, len(nps)),
        }
