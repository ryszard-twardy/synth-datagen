...
EXPORTED_COLUMNS: dict[str, list[str]] = {
    "accounts": ["account_id", "company_name", "industry", "company_size", "country", "signup_date", "status", "acquisition_channel"],
    "users": ["user_id", "account_id", "email", "role", "last_login_at", "is_active"],
    "subscriptions": ["subscription_id", "account_id", "plan_name", "billing_period", "start_date", "end_date", "monthly_amount", "status", "cancellation_reason"],
    "product_events": ["event_id", "account_id", "user_id", "event_name", "event_timestamp", "feature_category", "platform"],
    "invoices": ["invoice_id", "account_id", "invoice_date", "amount_due", "amount_paid", "currency", "status"],
    "support_tickets": ["ticket_id", "account_id", "category", "priority", "status", "satisfaction_rating", "resolution_minutes"],
...
class GeneratedTables:
    ...
    def row_counts(self) -> dict[str, int]:
        return {table_name: sum(len(batch) for batch in batches) for table_name, batches in self.tables.items()}
...
def _allocate_total(total: int, weights: np.ndarray, minimums: np.ndarray | None = None) -> np.ndarray:
    weights = np.asarray(weights, dtype=float)
    minimums = np.zeros(len(weights), dtype=int) if minimums is None else np.asarray(minimums, dtype=int)
    if total < int(minimums.sum()):
        raise ValueError("Requested total is smaller than the enforced minimum allocation")
...
def _month_starts(start_date: date, end_date: date) -> list[date]:
    starts = pd.date_range(pd.Timestamp(start_date).replace(day=1), pd.Timestamp(end_date).replace(day=1), freq="MS")
...
def _sample_dates_between(start_date: date, end_date: date, size: int, rng: np.random.Generator) -> list[date]:
...
class SaaSV3Engine:
    def __init__(self, config: SaaSV3Config, seed_override: int | None = None) -> None:
        ...
        self.plan_rank = {plan.name: index for index, plan in enumerate(self.config.plans)}
        self.size_by_name = {segment.name: segment for segment in self.config.company_sizes}
        self.country_by_name = {country.name: country for country in self.config.countries}
        self.industry_by_name = {industry.name: industry for industry in self.config.industries}
        self.channel_by_name = {channel.name: channel for channel in self.config.acquisition_channels}
    ...
    def _sample_configs(self, configs: list, size: int, rng: np.random.Generator, weight_attr: str = "weight") -> list:
        weights = np.array([getattr(item, weight_attr) for item in configs], dtype=float)
    ...
    def generate(self, mode: OutputMode = OutputMode.BOTH) -> GenerationResult:
        ...
        account_month_state = self._build_account_month_state(profiles, lifecycle, subscriptions)
        ...
        activity_budget = self._build_activity_budget(account_month_state, users_internal)
        invoices, invoice_internal = self._build_invoices(profiles, subscriptions, account_month_state)
        support_tickets, support_internal = self._build_support_tickets(profiles, account_month_state)
        nps_responses, nps_internal = self._build_nps_responses(profiles, account_month_state)
        product_event_batches = self._build_product_events(activity_budget, users_internal)
        ...
        clean = GeneratedTables(
            tables={
                ...
                "support_tickets": [support_tickets[EXPORTED_COLUMNS["support_tickets"]].copy()],
    ...
    def _build_account_profiles(self) -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        channel_choices = self._sample_configs(self.config.acquisition_channels, count, rng)
        total_days = (self.as_of_date - self.history_start).days
        signup_offsets = np.floor(rng.beta(2.0, 1.5, size=count) * total_days).astype(int)
        signup_dates = [self.history_start + timedelta(days=int(offset)) for offset in signup_offsets]
        ...
        for index in range(count):
            ...
            company_name = build_company_name(industry.name, country.company_suffixes, rng)
            ...
            expansion_score = float(np.clip(base_health * 0.45 + channel.expansion_adjustment + rng.normal(0.0, 0.45), -1.8, 1.8))
            retention_score = float(np.clip(base_health * 0.55 + channel.retention_adjustment + rng.normal(0.0, 0.40), -1.8, 1.8))
            payment_reliability = float(np.clip(0.79 + 0.07 * retention_score + rng.normal(0.0, 0.06), 0.35, 0.99))
            price_sensitivity = float(np.clip(0.58 - 0.12 * (segment.name == "Enterprise") + 0.08 * (segment.name == "SMB") + rng.normal(0.0, 0.05), 0.05, 0.95))
            user_target = int(
                round(
                    rng.integers(segment.user_count_min, segment.user_count_max + 1)
                    * np.clip(0.9 + 0.12 * industry.adoption_factor + 0.10 * _logistic(base_health), 0.55, 1.7)
                )
            )
            records.append(
                {
            ...
            billing_records.append(
                {
                    "account_id": account_id,
                    "preferred_billing_period": self._sample_billing_period(segment.name, rng),
                    "currency": country.currency,
                    "dunning_risk": round(float(np.clip(1.1 - payment_reliability + price_sensitivity / 3, 0.05, 1.5)), 4),
                    "payment_reliability": payment_reliability,
                }
            )
    ...
    def _build_lifecycle_scaffold(self, profiles: pd.DataFrame, billing_profile: pd.DataFrame) -> pd.DataFrame:
        rng = self._rng("lifecycle")
        age_days = (pd.Timestamp(self.as_of_date) - pd.to_datetime(profiles["signup_date"])).dt.days.clip(lower=30)
        account_count = len(profiles)
        target = self.config.row_targets.subscriptions
        churn_probs = (
            self.config.lifecycle.churn_base_probability
            * np.clip(1.05 - profiles["retention_score"] * 0.12 + profiles["price_sensitivity"] * 0.25, 0.25, 1.9)
            * np.clip(age_days / 365, 0.35, 1.6)
        ).clip(0.02, 0.62)
        churned = rng.random(account_count) < churn_probs.to_numpy(dtype=float)
        churned = np.where(age_days < self.config.lifecycle.minimum_tenure_days_before_churn, False, churned)
        if target >= account_count:
            extras = _allocate_total(
                target - account_count,
                weights=(age_days.to_numpy(dtype=float) * np.clip(1.2 + profiles["expansion_score"].to_numpy(dtype=float) + churn_probs.to_numpy(dtype=float), 0.15, None)),
            )
            subscription_counts = extras + 1
        else:
            subscription_counts = np.zeros(account_count, dtype=int)
            picks = rng.choice(account_count, size=target, replace=False, p=_normalize(age_days.to_numpy(dtype=float)))
        ...
        for index, profile in enumerate(profiles.itertuples(index=False)):
            ...
            churn_date = None
            if churned[index]:
                earliest = signup_date + timedelta(days=self.config.lifecycle.minimum_tenure_days_before_churn)
                latest = max(earliest, self.as_of_date - timedelta(days=15))
                span = max(0, (latest - earliest).days)
                churn_date = earliest + timedelta(days=int(rng.integers(0, span + 1)) if span else 0)
            records.append(
                {
                    ...
                    "preferred_billing_period": str(billing_lookup.loc[profile.account_id, "preferred_billing_period"]),
                    "incident_factor": round(float(1.0 + rng.binomial(1, self.config.lifecycle.incident_burst_probability) * rng.uniform(0.3, 1.1)), 4),
                }
            )
    ...
    def _sample_billing_period(self, size_name: str, rng: np.random.Generator) -> str:
        ...
        values = np.array([weights.get(name, 0.0) for name in ("monthly", "quarterly", "annual")], dtype=float)
        return str(rng.choice(["monthly", "quarterly", "annual"], p=_normalize(values)))

    def _sample_plan_for_account(self, profile: pd.Series, rng: np.random.Generator) -> str:
    ...
    def _next_plan_name(self, current_plan: str, profile: pd.Series, rng: np.random.Generator) -> str:
        ...
        if score < -0.45 and current_rank > 0:
            return self.plan_order[current_rank - 1]
        if current_rank < len(self.plan_order) - 1 and rng.random() < self.config.lifecycle.expansion_base_probability * 0.2:
            return self.plan_order[current_rank + 1]
        return current_plan

    def _build_subscriptions(self, profiles: pd.DataFrame, scaffold: pd.DataFrame) -> pd.DataFrame:
        ...
        for profile in profiles.to_dict(orient="records"):
            ...
            if count <= 0:
                continue
            signup_date = pd.Timestamp(profile["signup_date"]).date()
            active_end = pd.Timestamp(meta["churn_date"]).date() if pd.notna(meta["churn_date"]) else self.as_of_date
            total_window_days = max(0, (active_end - signup_date).days)
            max_anchor_shift = max(0, min(21, total_window_days - max(0, count - 1)))
            start_anchor = signup_date + timedelta(days=int(rng.integers(0, max_anchor_shift + 1)) if max_anchor_shift else 0)
            available_days = max(0, (active_end - start_anchor).days)
            transition_starts: list[date] = []
            if count > 1 and available_days >= count - 1:
                base_offsets = np.linspace(1, available_days, num=count, endpoint=True)[1:]
                offsets: list[int] = []
                previous = 0
                ...
                transition_starts = [start_anchor + timedelta(days=int(offset)) for offset in offsets]
            ...
            for period_index, period_start in enumerate(period_starts):
                if period_index > 0:
                    current_plan = self._next_plan_name(current_plan, pd.Series(profile), rng)
                    if rng.random() < 0.25:
                        current_billing = self._sample_billing_period(str(profile["company_size"]), rng)
                next_start = period_starts[period_index + 1] if period_index + 1 < len(period_starts) else None
                end_date = (next_start - timedelta(days=1)) if next_start else (active_end if bool(meta["churned"]) else None)
                status = "ended"
                cancellation_reason = "plan_change"
                if period_index == len(period_starts) - 1:
                    if bool(meta["churned"]):
                        status = "cancelled"
                        cancellation_reason = str(rng.choice(DEFAULT_CANCELLATION_REASONS))
                    elif float(profile["payment_reliability"]) < 0.58 and rng.random() < 0.16:
                        status = "past_due"
                ...
                rows.append(
                    {
                        ...
                        "end_date": end_date,
                        "monthly_amount": round(float(self.plan_by_name[current_plan].monthly_price), 2),
                        "status": status,
    ...
    def _build_account_month_state(self, profiles: pd.DataFrame, scaffold: pd.DataFrame, subscriptions: pd.DataFrame) -> pd.DataFrame:
        ...
        for profile in profiles.to_dict(orient="records"):
            ...
            active_end = pd.Timestamp(meta["churn_date"]).date() if pd.notna(meta["churn_date"]) else self.as_of_date
            months = _month_starts(pd.Timestamp(profile["signup_date"]).date(), active_end)
            sub_rows = grouped_subscriptions.get(profile["account_id"], pd.DataFrame())
            for month_start in months:
                month_end = _month_end(month_start, self.as_of_date)
                months_since_signup = max(0, (pd.Timestamp(month_start) - pd.Timestamp(profile["signup_date"])).days // 30)
                active_sub = None
                if not sub_rows.empty:
                    active_candidates = sub_rows[
                        (pd.to_datetime(sub_rows["start_date"]).dt.date <= month_end)
                        & (pd.to_datetime(sub_rows["end_date"]).dt.date.fillna(self.as_of_date) >= month_start)
                    ]
                    if not active_candidates.empty:
                        active_sub = active_candidates.iloc[-1]
                plan_name = active_sub["plan_name"] if active_sub is not None else self.plan_order[0]
                plan = self.plan_by_name[str(plan_name)]
                plan_rank = self.plan_rank[str(plan_name)]
                ramp = min(1.0, (months_since_signup + 1) / 4.0)
                seasonal = 1.0 + self.config.lifecycle.seasonal_strength * math.sin((month_start.month - 1) * math.tau / 12.0)
                near_churn_penalty = 0.0
                if bool(meta["churned"]) and pd.notna(meta["churn_date"]):
                    days_to_churn = (pd.Timestamp(meta["churn_date"]).date() - month_start).days
                    if days_to_churn <= 75:
                ...
                if active_sub is not None:
                    recent_change = (month_start - pd.Timestamp(active_sub["start_date"]).date()).days <= 45
                if bool(meta["churned"]) and pd.notna(meta["churn_date"]) and month_start >= pd.Timestamp(meta["churn_date"]).replace(day=1).date():
                ...
                state_event_factor = {"new": 0.60, "active": 1.00, "expanding": 1.18, "stagnant": 0.74, "at-risk": 0.52, "churned": 0.16}[lifecycle_state]
                state_ticket_factor = {"new": 1.20, "active": 0.85, "expanding": 0.95, "stagnant": 1.10, "at-risk": 1.45, "churned": 0.60}[lifecycle_state]
                size_segment = self.size_by_name[str(profile["company_size"])]
                active_users_estimate = int(max(1, round(float(profile["base_user_target"]) * ramp * max(0.25, health) * state_event_factor)))
                event_weight = max(0.01, size_segment.event_multiplier * float(profile["industry_adoption_factor"]) * plan.usage_multiplier * seasonal * max(0.22, health) * state_event_factor)
                ticket_pressure = max(0.05, size_segment.support_multiplier * float(profile["industry_support_factor"]) * plan.support_complexity * max(0.25, 1.2 - health) * state_ticket_factor * float(meta["incident_factor"]))
                nps_signal = float(np.clip(0.24 + 0.46 * health + 0.05 * float(profile["payment_reliability"]) + 0.03 * float(profile["industry_nps_bias"]) - 0.03 * ticket_pressure + rng.normal(0.0, 0.05), 0.0, 1.0))
                rows.append(
                    {
                        "account_id": profile["account_id"],
                        ...
                        "billing_period": active_sub["billing_period"] if active_sub is not None else str(meta["preferred_billing_period"]),
    ...
    def _apply_latest_account_status(self, profiles: pd.DataFrame, account_month_state: pd.DataFrame) -> pd.DataFrame:
        latest = (
        ...
        merged = profiles.drop(columns=["status"]).merge(latest, on="account_id", how="left")
        merged["status"] = merged["status"].fillna("new")
        return merged

    def _sample_user_roles(self, count: int, account_profile: pd.Series, rng: np.random.Generator) -> list[str]:
    ...
    def _build_users(self, profiles: pd.DataFrame, account_month_state: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
        rng = self._rng("users")
        latest_status = profiles.set_index("account_id")["status"]
        status_factor = latest_status.map({"new": 0.70, "active": 1.0, "expanding": 1.15, "stagnant": 0.75, "at-risk": 0.55, "churned": 0.25}).fillna(0.75)
        weights = profiles["base_user_target"].to_numpy(dtype=float) * status_factor.to_numpy(dtype=float)
        minimums = np.ones(len(profiles), dtype=int) if self.config.row_targets.users >= len(profiles) else np.zeros(len(profiles), dtype=int)
        counts = _allocate_total(self.config.row_targets.users, weights=weights, minimums=minimums)
        users_rows: list[dict[str, object]] = []
        users_internal_rows: list[dict[str, object]] = []
        active_end_by_account = account_month_state.groupby("account_id")["month_end"].max().to_dict()
        for index, profile in enumerate(profiles.to_dict(orient="records")):
            ...
            for user_index in range(user_count):
                ...
                if user_index == 0:
                    ...
                    create_date = _sample_dates_between(signup_date, max(signup_date, min(active_end, self.as_of_date)), 1, rng)[0]
                base_active_probability = {"new": 0.86, "active": 0.92, "expanding": 0.95, "stagnant": 0.72, "at-risk": 0.58, "churned": 0.16}[str(profile["status"])]
                is_active = bool(rng.random() < np.clip(base_active_probability * (0.78 + role.activity_multiplier * 0.18), 0.05, 0.99))
                last_login_at = None
                if is_active or rng.random() < 0.42:
                    anchor = min(self.as_of_date, active_end + timedelta(days=30))
                    lag_days = int(rng.exponential(max(1.0, role.login_half_life_days / max(0.4, float(profile["base_health"]) + 1.2))))
                    login_date = max(create_date, anchor - timedelta(days=lag_days))
                    last_login_at = min(datetime.combine(login_date, time(hour=int(rng.integers(6, 22)), minute=int(rng.integers(0, 60)), second=int(rng.integers(0, 60)))), datetime.combine(self.as_of_date, time(23, 59, 59)))
                user_id = self.id_factory.next_id("user_id")
                exported = {"user_id": user_id, "account_id": account_id, "email": email, "role": role_name, "last_login_at": last_login_at, "is_active": is_active}
                users_rows.append(exported)
                users_internal_rows.append({**exported, "user_created_at": create_date, "activity_multiplier": role.activity_multiplier, "utc_offset_hours": profile["utc_offset_hours"]})
        users = pd.DataFrame(users_rows)
        users_internal = pd.DataFrame(users_internal_rows)
        return users, users_internal

    def _build_activity_budget(self, account_month_state: pd.DataFrame, users_internal: pd.DataFrame) -> pd.DataFrame:
        ...
        weights = state["event_weight"].to_numpy(dtype=float) * np.maximum(1.0, state["user_count"].to_numpy(dtype=float))
        state["target_events"] = _allocate_total(self.config.row_targets.product_events, weights=weights)
        ...
        state["admin_share"] = np.clip(0.16 + (state["company_size"] == "Enterprise").astype(float) * 0.05, 0.10, 0.28)
        remainder = 1.0 - (state["core_share"] + state["advanced_share"] + state["admin_share"])
        state["integration_share"] = np.clip(remainder, 0.06, 0.38)
        total_shares = state[["core_share", "advanced_share", "admin_share", "integration_share"]].sum(axis=1)
        for column in ["core_share", "advanced_share", "admin_share", "integration_share"]:
            state[column] = state[column] / total_shares
        return state

    def _build_invoices(self, profiles: pd.DataFrame, subscriptions: pd.DataFrame, account_month_state: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        for subscription in subscriptions.to_dict(orient="records"):
            ...
            end_date = pd.Timestamp(subscription["end_date"]).date() if pd.notna(subscription["end_date"]) else self.as_of_date
            ...
            while cursor <= end_date:
                ...
                try:
                    ...
                    pass
                payment_reliability = float(profiles.loc[profiles["account_id"] == subscription["account_id"], "payment_reliability"].iloc[0])
                ...
                partial_cutoff = paid_cutoff + self.config.billing.partial_payment_probability
                overdue_cutoff = partial_cutoff + self.config.billing.overdue_probability
                if draw < paid_cutoff:
                    status = "paid"
                ...
                internal_rows.append(
                    {
                        **exported,
                        ...
                        "health_score": float(month_state["health_score"]) if month_state is not None and not isinstance(month_state, pd.DataFrame) else 0.64,
                    }
                )
        ...
        if len(invoices) > target > 0:
            weights = pd.to_datetime(invoices["invoice_date"]).astype("int64").to_numpy(dtype=float)
            keep = np.sort(rng.choice(len(invoices), size=target, replace=False, p=_normalize(weights)))
        ...
        if not invoices.empty:
            ...
            internal["subscription_start_date"] = _format_date_series(internal["subscription_start_date"])
            internal["subscription_end_date"] = _format_date_series(internal["subscription_end_date"])
        return invoices, internal

    def _sample_ticket_category(self, lifecycle_state: str, rng: np.random.Generator) -> str:
        categories = np.array(["onboarding", "billing", "bug", "integration", "permissions", "reporting", "feature_request"], dtype=object)
    ...
    def _build_support_tickets(self, profiles: pd.DataFrame, account_month_state: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
        rng = self._rng("support_tickets")
        counts = _allocate_total(self.config.row_targets.support_tickets, weights=account_month_state["ticket_pressure"].to_numpy(dtype=float))
        rows: list[dict[str, object]] = []
        internal_rows: list[dict[str, object]] = []
        for record, ticket_count in zip(account_month_state.to_dict(orient="records"), counts.tolist(), strict=False):
            for _ in range(int(ticket_count)):
                category = self._sample_ticket_category(str(record["lifecycle_state"]), rng)
                priority_weights = {"billing": np.array([0.18, 0.36, 0.32, 0.14]), "bug": np.array([0.10, 0.30, 0.40, 0.20]), "integration": np.array([0.12, 0.33, 0.36, 0.19])}.get(category, np.array([0.28, 0.38, 0.24, 0.10]))
                priority = str(rng.choice(["low", "medium", "high", "urgent"], p=_normalize(priority_weights)))
                status = str(rng.choice(["resolved", "closed", "pending", "open"], p=[0.52, 0.20, 0.16, 0.12]))
                resolution_base = {"low": 240, "medium": 720, "high": 1_440, "urgent": 3_600}[priority]
                resolution_minutes = int(max(5, round(rng.lognormal(math.log(resolution_base), 0.55))))
                satisfaction = None
                if status in {"resolved", "closed"}:
                    score = 4.2 - record["ticket_pressure"] * 0.7 + rng.normal(0.0, 0.8)
                    satisfaction = int(np.clip(round(score), 1, 5))
                opened_at = datetime.combine(_sample_dates_between(pd.Timestamp(record["month_start"]).date(), pd.Timestamp(record["month_end"]).date(), 1, rng)[0], time(hour=int(rng.integers(8, 19)), minute=int(rng.integers(0, 60))))
                closed_at = opened_at + timedelta(minutes=resolution_minutes)
                ticket_id = self.id_factory.next_id("ticket_id")
                exported = {"ticket_id": ticket_id, "account_id": record["account_id"], "category": category, "priority": priority, "status": status, "satisfaction_rating": satisfaction, "resolution_minutes": resolution_minutes}
                rows.append(exported)
                internal_rows.append({**exported, "opened_at": opened_at, "closed_at": closed_at, "month_start": record["month_start"]})
        return pd.DataFrame(rows), pd.DataFrame(internal_rows)

    def _build_nps_responses(self, profiles: pd.DataFrame, account_month_state: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
        rng = self._rng("nps")
        state_lookup = {(row.account_id, row.month_start): row for row in account_month_state.itertuples(index=False)}
        ...
        for profile in profiles.to_dict(orient="records"):
            ...
            while cursor <= self.as_of_date:
                ...
                if state is not None and state.lifecycle_state != "churned":
                    candidates.append({"account_id": profile["account_id"], "survey_date": cursor, "nps_signal": float(state.nps_signal), "weight": max(0.05, float(state.nps_signal) + 0.20)})
                cursor = cursor + timedelta(days=int(rng.integers(110, 181)))
        if not candidates:
            return pd.DataFrame(columns=EXPORTED_COLUMNS["nps_responses"]), pd.DataFrame()
        candidate_df = pd.DataFrame(candidates)
        target = min(self.config.row_targets.nps_responses, len(candidate_df))
        indices = rng.choice(len(candidate_df), size=target, replace=False, p=_normalize(candidate_df["weight"].to_numpy(dtype=float)))
        selected = candidate_df.iloc[np.sort(indices)].copy().reset_index(drop=True)
        selected["response_id"] = [self.id_factory.next_id("response_id") for _ in range(len(selected))]
        selected["score"] = [int(np.clip(round(2.0 + 8.0 * float(signal) + rng.normal(0.0, 1.7)), 0, 10)) for signal in selected["nps_signal"].tolist()]
    ...
    def _sample_event_payload(self, plan_name: str, size: int, rng: np.random.Generator) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        plan_rank = self.plan_rank[plan_name]
        eligible: list[EventTypeConfig] = []
        weights: list[float] = []
        for event in self.config.event_taxonomy:
            if event.min_plan is not None and self.plan_rank[event.min_plan] > plan_rank:
                continue
            eligible.append(event)
            weights.append(event.weight)
        picks = rng.choice(len(eligible), size=size, p=_normalize(np.array(weights, dtype=float)))
        ...
        for event_index in np.unique(picks):
            ...
            platform_weights = np.array(list(event.platform_weights.values()), dtype=float)
            platforms[mask] = rng.choice(platform_names, size=int(mask.sum()), p=_normalize(platform_weights))
        return event_names, categories, platforms

    def _sample_event_timestamps(self, start_date: date, end_date: date, utc_offset_hours: int, size: int, rng: np.random.Generator) -> pd.Series:
        days = pd.date_range(start=start_date, end=end_date, freq="D")
        day_weights = np.array([1.15 if day.weekday() < 5 else 0.35 for day in days], dtype=float)
        selected_days = days[rng.choice(len(days), size=size, p=_normalize(day_weights))]
        business_draw = rng.random(size)
        local_hours = np.where(business_draw < 0.78, rng.integers(7, 19, size=size), np.where(business_draw < 0.94, rng.integers(19, 23, size=size), rng.integers(0, 7, size=size)))
        minutes = rng.integers(0, 60, size=size)
        seconds = rng.integers(0, 60, size=size)
        local_ts = pd.to_datetime(selected_days.date) + pd.to_timedelta(local_hours, unit="h") + pd.to_timedelta(minutes, unit="m") + pd.to_timedelta(seconds, unit="s")
        return pd.Series(local_ts - pd.to_timedelta(utc_offset_hours, unit="h"))

    def _build_product_events(self, activity_budget: pd.DataFrame, users_internal: pd.DataFrame) -> list[pd.DataFrame]:
        rng = self._rng("product_events")
        users_by_account = {account_id: group.sort_values("user_created_at").reset_index(drop=True) for account_id, group in users_internal.groupby("account_id", sort=False)}
        ...
        for _, month_rows in activity_budget.groupby("month_start", sort=True):
            ...
            for record in month_rows.to_dict(orient="records"):
                ...
                eligible_users = users.loc[pd.to_datetime(users["user_created_at"]).dt.date <= month_end].copy()
                if eligible_users.empty:
                    continue
                weights = eligible_users["activity_multiplier"].to_numpy(dtype=float) * np.where(eligible_users["is_active"], 1.25, 0.35)
                selected_users = eligible_users.iloc[rng.choice(len(eligible_users), size=event_count, replace=True, p=_normalize(weights))].reset_index(drop=True)
                event_names, categories, platforms = self._sample_event_payload(str(record["plan_name"]), event_count, rng)
                timestamps = self._sample_event_timestamps(pd.Timestamp(record["month_start"]).date(), month_end, int(record["utc_offset_hours"]), event_count, rng)
                timestamps = pd.Series(
                    np.maximum(
                        timestamps.to_numpy(dtype="datetime64[ns]"),
                        pd.to_datetime(selected_users["user_created_at"]).to_numpy(dtype="datetime64[ns]"),
                    )
                )
                month_frames.append(
                    pd.DataFrame(
                        {
                            "event_id": self.id_factory.next_ids("event_id", event_count),
