...
def test_kupferkanne_discount_variation_uses_line_level_formula_and_weighted_order_discount() -> (
    None
):
    clean = build_clean_kupferkanne_frames(
        _small_config(), seed=42, discount_variation=True
    )
    ...
    weighted["expected_discount"] = (
        weighted["discount_value"] / weighted["gross"]
    ).round(4)
    actual_discount = orders.set_index("OrderID")["OrderDiscountPct"].round(4)
    assert np.allclose(
        actual_discount.to_numpy(), weighted["expected_discount"].to_numpy(), atol=1e-4
    )
...
def test_kupferkanne_discount_variation_spreads_margin_by_value_tier() -> None:
    ...
    lines["cost"] = pd.to_numeric(lines["Quantity"], errors="coerce") * pd.to_numeric(
        lines["UnitCost"], errors="coerce"
    )
    margin = (
        lines.groupby("customer_archetype")[["revenue", "cost"]]
        .sum()
        .pipe(lambda df: (df["revenue"] - df["cost"]) / df["revenue"])
    )
    high_value_mean = float(
        margin.loc[["Coffee Regulars", "Premium Enthusiasts", "Power Buyers"]].mean()
    )
    low_value_mean = float(
        margin.loc[["Gift Buyers", "One-Time Buyers", "New Arrivals"]].mean()
    )
