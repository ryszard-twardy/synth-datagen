...
def test_saas_account_mrr_matches_active_subscriptions(tmp_path) -> None:
    ...
    active = dfs["subscriptions"].loc[
        dfs["subscriptions"]["status"].isin(["active", "past_due", "trialing", "paused"])
    ]
...
def test_saas_users_and_activity_follow_entity_lifecycle(tmp_path) -> None:
    dfs, _ = generate_scenario_dfs(Scenario.SAAS, tmp_path)
    users = dfs["users"].merge(dfs["accounts"][["account_id", "domain", "created_at"]], on="account_id", how="left", suffixes=("", "_account"))
    assert (pd.to_datetime(users["created_at"]) >= pd.to_datetime(users["created_at_account"])).all()
    assert users.apply(lambda row: str(row["email"]).endswith(f"@{row['domain']}"), axis=1).all()

    events = dfs["events"].merge(users[["user_id", "created_at"]], on="user_id", how="left")
    assert (pd.to_datetime(events["occurred_at"]) >= pd.to_datetime(events["created_at"])).all()

    usage = dfs["feature_usage"].merge(users[["user_id", "created_at"]], on="user_id", how="left")
    assert (pd.to_datetime(usage["used_at"]) >= pd.to_datetime(usage["created_at"])).all()
...
def test_saas_feature_usage_respects_plan_minimum(tmp_path) -> None:
    dfs, _ = generate_scenario_dfs(Scenario.SAAS, tmp_path)
    plan_rank = {"free": 0, "starter": 1, "growth": 2, "professional": 3, "enterprise": 4}
    usage = (
        dfs["feature_usage"]
    ...
    subscriptions["ended_at"] = pd.to_datetime(subscriptions["ended_at"]).fillna(pd.Timestamp.max.normalize())
    plan_by_row = []
    ...
    assert pd.Series(plan_by_row).map(plan_rank).ge(usage["plan_min"].map(plan_rank)).all()
