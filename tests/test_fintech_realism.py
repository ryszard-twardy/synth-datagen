...
def test_fintech_accounts_follow_customer_lifecycle(tmp_path) -> None:
    ...
    merged = dfs["accounts"].merge(
        dfs["customers"][["customer_id", "created_at"]].rename(
    ...
    assert (
        pd.to_datetime(merged["opened_at"])
        >= pd.to_datetime(merged["customer_created_at"])
    ).all()
    closed_mask = merged["closed_at"].notna()
    assert (
        pd.to_datetime(merged.loc[closed_mask, "closed_at"])
        >= pd.to_datetime(merged.loc[closed_mask, "opened_at"])
    ).all()
...
def test_fintech_transactions_are_chronological_per_account(tmp_path) -> None:
    ...
    assert (
        created[closed_mask]
        <= pd.to_datetime(transactions.loc[closed_mask, "closed_at"])
    ).all()

    for _, group in transactions.sort_values(
        ["account_id", "created_at", "transaction_id"]
    ).groupby("account_id"):
        prev_balance = None
        for row in group.itertuples(index=False):
            if prev_balance is not None:
                assert round(prev_balance + float(row.amount), 2) == round(
                    float(row.balance_after), 2
                )
...
def test_fintech_merchants_and_loans_are_plausible(tmp_path) -> None:
    ...
    assert (
        transactions.loc[
            transactions["tx_type"].isin(["purchase", "refund"]), "merchant_id"
        ]
        .notna()
        .all()
    )
...
def test_fintech_cards_follow_account_lifecycle(tmp_path) -> None:
    dfs, _ = generate_scenario_dfs(Scenario.FINTECH, tmp_path)
    cards = dfs["cards"].merge(
        dfs["accounts"][["account_id", "opened_at", "is_active"]],
        on="account_id",
        how="left",
    )
    assert (
        pd.to_datetime(cards["issue_date"])
        >= pd.to_datetime(cards["opened_at"]).dt.normalize()
    ).all()
    expired_active = (
        pd.to_datetime(cards["expiry_date"]) < pd.Timestamp("2025-12-31")
    ) & cards["is_active_x"]
